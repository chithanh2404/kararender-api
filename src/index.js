...
// FIXED REGEX FOR FONTS - case insensitive, support .ttf .TTF .otf .woff2
function isFontFile(name) {
  return /\.(ttf|otf|woff2|woff)$/i.test(name);
}
...
<title>KaraRender Admin v3.9 - Font Fixed</title>
<style>
...
<div style="max-width:1300px;margin:0 auto;padding:20px">
  <h1 style="font-size:22px;font-weight:900;margin-bottom:16px">👑 KaraRender Admin v3.9 - Font Case Fixed (TTF/OTF)</h1>
  ...
  <div class="grid2" style="margin-bottom:20px">
    <div class="card" style="border-color:#10b981;background:rgba(16,185,129,0.08)">
      ...
      <p style="font-size:11px;color:#94a3b8">Fix: hỗ trợ .TTF uppercase, .OTF, .woff2 - regex đã sửa</p>
    ...
    <div class="card" style="border-color:#06b6d4;background:rgba(6,182,214,0.08)">
      <h3 style="color:#22d3ee">🌐 Languages & Secure</h3>
...
app.get('/api/admin/debug', async (req, res) => {
  ...
  let usersCount=0, fontsCount=0, effectsInfo={ exists:false }, langCount=0;
  ...
  res.json({
    version: 'v3.9 font case fixed',
    env: {
      FONTS_FOLDER_ID: process.env.FONTS_FOLDER_ID || 'default',
      EFFECTS_FOLDER_ID: process.env.EFFECTS_FOLDER_ID || process.env.EFFECTS_FILE_ID || 'default',
...
app.post('/api/admin/import-fonts', async (req,res)=>{
  try{
    ...
    const files=await listFilesInFolder(folderId);
    // FIXED: case-insensitive regex, support .TTF .OTF .ttf .otf .woff2 .woff
    const fontFiles=files.filter(f => /\.(ttf|otf|woff2|woff)$/i.test(f.name));
    if(!fontFiles.length) return res.json({ status:'error', message:`Folder có ${files.length} file nhưng không có font (regex lỗi)`, totalFiles: files.length, sample: files.slice(0,10).map(f=>f.name), debug: 'Testing isFont: '+files.slice(0,3).map(f=> f.name + '->' + (/\.(ttf|otf|woff2|woff)$/i.test(f.name))).join(', ') });
    ...
    for(const f of fontFiles){
      try{
        ...
        else if(content && content.data) buffer=Buffer.from(content.data);
        else buffer=Buffer.from(content);
        // Fix: getFileContent returns arraybuffer, need to handle correctly
        // The drive service returns res.data as arraybuffer, we already converted to Buffer in getFileContent? Let's ensure
        if (!buffer || buffer.length < 100) {
          // Try get as string? No, font is binary, need proper fetch
    ...
    res.json({ status:'success', message:`Import fonts: ${imported} imported, ${skipped} đã có, ${errors.length} lỗi`, imported, skipped, totalFiles: fontFiles.length, totalInFolder: files.length, errors: errors.slice(0,15) });
...
app.all('/exec', async (req,res)=>{
  ...
  try{
    ...
    switch(action){
      case 'verify':{
        const p=decodeOldToken(params.token||'');
        if(!p) return sendJSONP({ success:false });
        return sendJSONP({ success:true, valid:true, token:params.token, user:{ email:p.email, fullName:p.fullName||p.full_name, role:p.role||'USER', isVip:p.isVip||p.is_vip, is_vip:p.isVip||p.is_vip, isAdmin:p.role==='ADMIN' } });
      }
      case 'login':{
      ...
      case 'getFonts':{
        try{
          const { data } = await supabaseAdmin.storage.from('fonts').list();
          if(data&&data.length){
            // FIXED REGEX HERE TOO
            const fonts=data.filter(f=> /\.(ttf|otf|woff2|woff)$/i.test(f.name)).map(f=>({ name:f.name.replace(/\.[^/.]+$/,''), url: `${config.SUPABASE_URL}/storage/v1/object/public/fonts/${f.name}` }));
...
app.get('/', (req,res)=>res.json({ status:'KaraRender API v3.9 FONT FIXED', uptime:process.uptime() }));
app.get('/health',(req,res)=>res.json({ ok:true, version:'3.9' }));
app.listen(PORT,()=>console.log(`🚀 KaraRender v3.9 FONT FIXED listening on ${PORT}`));
