...
async function fetchFromDropboxDirect() {
  // FIXED: Add timestamp to bust Dropbox + Railway cache
  const bust = '_t=' + Date.now();
  const urls = [
    DROPBOX_DIRECT_URL + (DROPBOX_DIRECT_URL.includes('?') ? '&' : '?') + bust,
    DROPBOX_DIRECT_URL_ALT + (DROPBOX_DIRECT_URL_ALT.includes('?') ? '&' : '?') + bust
  ];
  for (const url of urls) {
    try {
      const res = await fetch(url, { method: 'GET', cache: 'no-store', headers: { 'User-Agent': 'KaraRender/5.5-FIXED-' + Date.now(), 'Cache-Control': 'no-cache, no-store', 'Pragma': 'no-cache' } });
      if (!res.ok) continue;
      const text = await res.text();
      if (!text || text.length < 1000) continue;
      if (!text.includes('KaraSecureRender') && !text.includes('vmX')) continue;
      console.log('[Secure-v5.5-Dropbox] Success', url.slice(0,80)+'...', 'length', text.length, 'hasV2.3:', text.includes('2.3-smooth-fix'), 'hasKara:', text.includes('KaraSecureRender'), 'first100:', text.slice(0,100));
      return text;
    } catch (e) {
...
const RAM_CACHE_TTL = 0; // FIXED: disable RAM cache to always fetch fresh from Dropbox - was 30min causing old module
...
async function getSecureRenderModuleContent_V55() {
  // FIXED: Disabled RAM cache - always fetch fresh from Dropbox to avoid old module
  if (false && cachedModuleInRAM && Date.now() - cacheTimeInRAM < RAM_CACHE_TTL) {
    console.log('[Secure-v5.5] RAM cache hit (DISABLED), length', cachedModuleInRAM.length);
    return cachedModuleInRAM;
  }
  // Force clear old cache if exists
  if(cachedModuleInRAM && Date.now() - cacheTimeInRAM > 1000){
    console.log('[Secure-v5.5] Clearing old RAM cache to fetch fresh, old length', cachedModuleInRAM.length);
...
app.post('/api/admin/clear-secure-cache', async (req,res)=>{
  try{
    ...
    cachedModuleInRAM = null;
    cacheTimeInRAM = 0;
    // FIXED: Immediately fetch fresh from Dropbox after clear
    let fresh = null;
    try{
      fresh = await fetchFromDropboxDirect();
      if(fresh){
        console.log('[ClearCache] Fetched fresh after clear, length', fresh.length, 'isNewV2.3:', fresh.includes('2.3-smooth-fix'));
      }
    }catch(e){ console.log('[ClearCache] Fetch fresh error', e.message); }
    res.json({ 
      status:'success', 
      message: hadCache ? `Đã xóa RAM cache ${length} chars` + (fresh ? `, đã tải mới ${fresh.length} chars, isV2.3=${fresh.includes('2.3-smooth-fix')}` : ', chưa tải mới được') : 'Không có cache' + (fresh ? `, đã tải mới ${fresh.length} chars` : ''), 
      hadCache, 
      length,
      freshLength: fresh?.length || 0,
      isNewV2_3: fresh?.includes('2.3-smooth-fix') || false,
      first100: fresh?.slice(0,100) || ''
    });
  }catch(e){ res.status(500).json({ status:'error', message:e.message }); }
});
